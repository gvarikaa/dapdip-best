...
import { prisma } from "@/prisma";
import { imagekit } from "@/utils";
import { clerkClient as getClerkClient } from "@clerk/nextjs/server"; // clerkClient ფუნქციის სახით
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const clerk = await getClerkClient(); // clerkClient-ს გაშვება
    ...
    if (avatarFile && avatarFile.size > 0) {
      ...
      if (avatarResult && avatarResult.filePath) {
        ...
        try {
          // unsafeMetadata-ის გამოყენებით ვახდენთ სურათის განახლებას
          await clerk.users.updateUser(userId, {
            unsafeMetadata: {
              imageUrl: avatarResult.url // imageUrl განახლდება unsafeMetadata-ში
            }
          });
        } catch (clerkError) {
          console.error("Clerk ავატარის განახლების შეცდომა:", clerkError);
        }
      }
    }
    ...
    if (coverFile && coverFile.size > 0) {
      ...
      if (coverResult && coverResult.filePath) {
        ...
        try {
          await clerk.users.updateUser(userId, {
            unsafeMetadata: {
              cover: coverResult.url // unsafeMetadata-ში ქავერის განახლება
            }
          });
        } catch (clerkError) {
          console.error("Clerk ქავერის metadata განახლების შეცდომა:", clerkError);
        }
      }
    }
